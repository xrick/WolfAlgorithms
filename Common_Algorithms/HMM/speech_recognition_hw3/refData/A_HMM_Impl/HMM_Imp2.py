...
class HMM:
    ...
    def generateData(self, T):
        ...
        observationsStates[0] = _getFromProbs(
            self.B[hiddenStates[0]])  # 产生第一个observationStates
        for t in range(1, T):
    ...
    def viterbi(self, observationsSeq):
        ...
        for t in range(1, T):
            for n in range(N):
                probs = dpMatrix[t-1, :] * self.A[:, n] * \
                    self.B[n, observationsSeq[t]]
    ...
    def _forward(self, observationsSeq):
        ...
        for t in range(1, T):
            for n in range(0, N):
                alpha[t, n] = np.dot(alpha[t-1, :], self.A[:, n]) * \
                    self.B[n, observationsSeq[t]]  # 使用内积简化代码
        return alpha
    ...
    def _backward(self, observationsSeq):
        ...
        for t in reversed(range(T-1)):
            for n in range(N):
                beta[t, n] = np.sum(
                    self.A[n, :] * self.B[:, observationsSeq[t+1]] * beta[t+1, :])
        return beta
    ...
    def baumWelch(self, observationsSeq, criterion=0.001):
        ...
        while True:
            ...
            for t in range(T-1):
                ...
                for i in range(N):
                    molecular = alpha[t, i] * self.A[i, :] * \
                        self.B[:, observationsSeq[t+1]] * beta[t+1, :]
